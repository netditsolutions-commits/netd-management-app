...
const App = {
  ...
  async saveSettingsForm(e) {
    ...
    const settings = {
      companyName: getVal('set-company-name', 'NETD IT SOLUTIONS'),
      tagline: getVal('set-tagline', ''),
      phone: getVal('set-phone', ''),
      email: getVal('set-email', ''),
      address: getVal('set-address', ''),
      bankInfo: getVal('set-bank-info', ''),
      taxId: getVal('set-tax-id', ''),
      currency: getVal('set-currency', 'LAK')
    };

    if (window.db) {
      await window.db.saveSettings(settings);
      this.showToast(typeof t === 'function' ? t('toast_success_save') : 'ບັນທຶກຂໍ້ມູນບໍລິສັດສຳເລັດແລ້ວ!', 'success');
  ...
  initSettings() {
    ...
    if (form) {
      form.onsubmit = (e) => this.saveSettingsForm(e);
    }
  },
