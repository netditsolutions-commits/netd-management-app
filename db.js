      ...
      if (cached) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          if (parsed.settings && Object.keys(parsed.settings).length > 0) {
            if (parsed.settings.phone && parsed.settings.phone.includes('+856 20 5555 8899')) {
              delete parsed.settings.phone;
            }
            if (parsed.settings.address && parsed.settings.address.includes('Phonsinuan Village')) {
              delete parsed.settings.address;
            }
            if (parsed.settings.email && parsed.settings.email.includes('contact@netd-it.la')) {
              delete parsed.settings.email;
            }
            if (parsed.settings.bankInfo && parsed.settings.bankInfo.includes('010-12-00-01234567-001')) {
              delete parsed.settings.bankInfo;
            }
